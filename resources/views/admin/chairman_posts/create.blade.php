...
@section('title', 'Tulis Karya/Artikel Baru Ketua')
@section('page_title', 'Tulis Arsip Ketua')
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-400/10 border border-amber-400/20 text-amber-600 dark:text-amber-400 text-xs font-bold uppercase tracking-wider mb-2">
                ...
                <span>Formulir Penulisan Arsip</span>
            </div>
            <h2 class="text-xl sm:text-2xl font-black text-[#0B132B] dark:text-white">Tambah Karya Tulis & Esai Pemikiran Ketua</h2>
            <p class="text-xs text-slate-600 dark:text-slate-400 mt-1 font-medium">
                Artikel yang disimpan akan langsung terindeks di halaman portofolio eksekutif <a href="{{ route('chairman.archive.index') }}" target="_blank" class="text-blue-600 dark:text-amber-400 font-bold hover:underline">/wardoyo</a>
            </p>
        </div>
    ...
    <!-- Form Container -->
    <form action="{{ route('admin.chairman_posts.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="p-6 sm:p-8 rounded-2xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:col-span-8">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required class="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-sm font-semibold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 outline-none shadow-xs" placeholder="Masukkan judul artikel yang berbobot dan informatif...">
                </div>
                ...
                <div class="md:col-span-4">
                    ...
                    <input type="text" name="category" list="categoryList" value="{{ old('category', 'Jurnalistik & Pers') }}" required class="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-sm font-semibold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 outline-none shadow-xs" placeholder="Pilih atau ketik kategori baru...">
                ...
                <div class="md:col-span-4">
                    ...
                    <p class="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                        Bisa disetel ke tanggal lampau (*backdate*) arsip lama.
                    </p>
                </div>
                ...
                <div class="md:col-span-4">
                    ...
                    <input type="url" name="original_url" value="{{ old('original_url') }}" class="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-xs font-medium text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 outline-none shadow-xs" placeholder="https://wardianst.wordpress.com/...">
                    <p class="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                        Tautan artikel asli sebagai rujukan bila diperlukan.
                    </p>
                </div>
                ...
                <div class="md:col-span-12">
                    ...
                    <textarea name="excerpt" rows="2" class="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-xs font-medium text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 outline-none shadow-xs" placeholder="1-2 kalimat pengantar tulisan yang tampil di kartu katalog (opsional, diambil otomatis dari awal artikel bila kosong)...">{{ old('excerpt') }}</textarea>
                </div>
