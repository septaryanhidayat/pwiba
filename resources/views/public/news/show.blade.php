...
<div class="py-12 bg-slate-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex items-center gap-2 text-xs text-slate-500 mb-6">
            ...
            <span>/</span>
            <a href="{{ route('news.index') }}" class="hover:text-amber-600">Berita</a>
            <span>/</span>
            <span class="text-slate-800 font-semibold truncate max-w-xs">{{ $post->judul }}</span>
        </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div class="lg:col-span-8">
                <article class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-10 shadow-sm">
                    
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-lg text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200 mb-4">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-4 py-4 border-y border-slate-100 text-xs text-slate-500 mb-8">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold">
                                <i class="fa-solid fa-user-pen text-xs text-amber-400"></i>
                            </div>
                            <div>
                                <span class="block font-bold text-slate-800">{{ $post->penulis }}</span>
                    ...
                    <!-- Featured Image -->
                    <div class="rounded-2xl overflow-hidden aspect-[16/9] bg-slate-100 mb-8 shadow-md">
                    ...
                    <div class="mt-10 pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <span class="text-xs font-bold text-slate-700 uppercase">Bagikan Berita Ini:</span>
                        <div class="flex items-center gap-2">
                            <a href="https://api.whatsapp.com/send?text={{ urlencode($post->judul . ' ' . url()->current()) }}" target="_blank" class="px-3.5 py-2 rounded-xl text-xs font-semibold bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors flex items-center gap-1.5">
                                <i class="fa-brands fa-whatsapp text-sm"></i> WhatsApp
                            </a>
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" target="_blank" class="px-3.5 py-2 rounded-xl text-xs font-semibold bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors flex items-center gap-1.5">
                                <i class="fa-brands fa-facebook text-sm"></i> Facebook
                            </a>
                            <a href="https://twitter.com/intent/tweet?text={{ urlencode($post->judul) }}&url={{ urlencode(url()->current()) }}" target="_blank" class="px-3.5 py-2 rounded-xl text-xs font-semibold bg-sky-50 text-sky-700 hover:bg-sky-100 transition-colors flex items-center gap-1.5">
                        ...
                        @foreach($relatedPosts as $rp)
                            <a href="{{ route('news.show', $rp->slug) }}" class="flex gap-3 group">
                                ...
                                <div class="min-w-0 flex-grow">
                                    <h4 class="text-xs font-bold text-slate-800 group-hover:text-blue-600 line-clamp-2 leading-snug">
                                        {{ $rp->judul }}
                                    </h4>
