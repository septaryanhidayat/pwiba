@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Navigasi Halaman" class="w-full flex flex-col sm:flex-row items-center justify-between gap-3 px-2 py-2 sm:px-1">
        ...
        <!-- Summary Info (Left on Desktop, Below Controls on Mobile) -->
        ...
        <div class="order-1 sm:order-2 flex items-center justify-center">
            ...
            <!-- Mobile View (< sm): Compact Navigation (Never Overflows) -->
            <div class="flex sm:hidden items-center gap-1.5 p-1 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs">
                {{-- Previous Page Link --}}
                @if ($paginator->onFirstPage())
                    <span class="w-8 h-8 rounded-xl flex items-center justify-center text-slate-300 dark:text-slate-600 cursor-not-allowed text-xs" aria-disabled="true" aria-label="Sebelumnya">
                ...
                <span class="px-3 py-1 rounded-xl bg-blue-50 dark:bg-slate-800/80 border border-blue-100 dark:border-slate-700 text-xs font-bold text-blue-700 dark:text-amber-400">
                    Hal <span class="font-black">{{ $paginator->currentPage() }}</span> / {{ $paginator->lastPage() }}
                </span>
                ...
                @else
                    <span class="w-8 h-8 rounded-xl flex items-center justify-center text-slate-300 dark:text-slate-600 cursor-not-allowed text-xs" aria-disabled="true" aria-label="Selanjutnya">
                        <i class="fa-solid fa-chevron-right"></i>
                    </span>
                @endif
            </div>

            <!-- Desktop View (>= sm): Full Numbered Pill Bar -->
            <div class="hidden sm:inline-flex items-center gap-1 p-1 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs">
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <span class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center bg-blue-600 text-white font-black text-xs sm:text-sm shadow-xs" aria-current="page">
                                    {{ $page }}
                                </span>
                            @else
                                <a href="{{ $url }}" 
